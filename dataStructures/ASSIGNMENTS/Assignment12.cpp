...
int Graph::DFS(vertex *VERTEX, int distance_traveled){
    if(!VERTEX->visited)
    ...
    for(int m=0; m<vertices.size(); m++){
        for(int x=0; x<vertices[m].adj.size(); x++){
			vertices[m].adj[x].visited = false;
		}
    }
    
    for(int x=0; x<VERTEX->adj.size(); x++){
        if(!VERTEX->adj[x].visited){
            cout << VERTEX->adj[x].weight << endl;
            distance_traveled += VERTEX->adj[x].weight;
            dists.push_back(VERTEX->adj[x].weight);
            VERTEX->adj[x].visited = true;
            distance_traveled = DFS(VERTEX->adj[x].v, distance_traveled);
        }
    }
...
int main(){
    ...
    while(menuON){
        if(US == 1){
            ...
            cout << "Enter name of City to start DFS traversal on" << endl;
            string t2;
            cin.ignore();
            getline(cin, t2);
            vertex V = G1.Search(t2);
            vertex *V1 = &V;
            int d = 0;
            int dt = G1.DFS(V1, d);
            cout << "distance traveled in DFS: " << dt << endl;
            cout << "Depth first traversal completed" << endl;
            setALLVisistedFalse();
            cin >> US;
        }else if(US == 5){
            cout << "Goodbye!" << endl;
